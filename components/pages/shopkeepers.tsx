...
export function Shopkeepers() {
  ...
  const handlePayment = async (shopkeeperId: number) => {
    ...
    try {
      ...
      setShopkeepers((prev) =>
        prev.map((s) => {
          if (s.id === shopkeeperId) {
            ...
            const newBalance = s.balance < 0 ? s.balance + amount : s.balance
            ...
            const updatedPendingReceipts = s.pendingReceipts.map(receipt => {
      ...
      // await syncManager.recordPayment(shopkeeperId, amount)
